...
class TestCheckoutFW(TestCase):
    ...
    def test_checkout_fw(self):
        ...
        self.lp.add_wf(
            Firework(ScriptTask.from_str(shell_cmd='echo "hello 2"', parameters={"stdout_file": "task.out"}), fw_id=2)
        )
...
class TestEarlyExit(TestCase):
    ...
    def test_early_exit(self):
        ...
        fw2 = Firework(ScriptTask.from_str(shell_cmd=script_text, parameters={"stdout_file": "task.out"}), fw_id=2)
        fw3 = Firework(ScriptTask.from_str(shell_cmd=script_text, parameters={"stdout_file": "task.out"}), fw_id=3)
        fw4 = Firework(ScriptTask.from_str(shell_cmd=script_text, parameters={"stdout_file": "task.out"}), fw_id=4)
