...
from fireworks import FiretaskBase, Firework, FWAction, LaunchPad, PyTask, Workflow, explicit_serialize
...
class IfNonstrictTask(FiretaskBase):
    required_params = ["condition", "input_1", "fw_id_1", "input_2", "fw_id_2", "output", "detour_name"]
    ...
    def run_task(self, fw_spec):
        ...
        fwk = Firework(name=self["detour_name"], tasks=SummationTask(inputs=[inp], output=self["output"]))
