...
def lpad():
    ...
    stats_parser = subparsers.add_parser('stats', help='Various statistics, type "lpad stats -h" for more.',
                    parents=[parent_parser])
    stats_parent_parser=ArgumentParser(add_help=False)
    stats_parent_parser.add_argument('-s', '--start', help="The start time (inclusive) to query in isoformat (YYYY-MM-DDTHH:MM:SS.mmmmmm). Default is 30 days from now.")
    stats_parent_parser.add_argument('-e', '--end', help="The end time (exclusive) to query in isoformat (YYYY-MM-DDTHH:MM:SS.mmmmmm). Default is now.")
    stats_parent_parser.add_argument('-w', '--weeks', help="Time difference in weeks to calculate start time from end time.", type=float, default=0)
    stats_parent_parser.add_argument('-d', '--days', help="Time difference in days to calculate start time from end time.", type=float, default=30)
    stats_parent_parser.add_argument('-o', '--hours', help="Time difference in hours to calculate start time from end time.", type=float, default=0)
    stats_parent_parser.add_argument('-m', '--minutes', help="Time difference in minutes to calculate start time from end time.", type=float, default=0)
    stats_parent_parser.add_argument('-q', '--query', help="Additional Pymongo queries to filter entries for process.")
    stats_subparser = stats_parser.add_subparsers(title="action", dest="action_command")

    fw_report_parser = stats_subparser.add_parser('get_fw_report', help='Get a report for fireworks', parents=[stats_parent_parser])
    fw_report_parser.add_argument('-f', '--time_field', default="time_end")
    fw_report_parser.set_defaults(func=report)
    launch_report_parser = stats_subparser.add_parser('get_launch_report', help='Get a report for luanches', parents=[stats_parent_parser])
    ...
    wf_report_parser = stats_subparser.add_parser('get_wf_report', help='Get a report for workflows', parents=[stats_parent_parser])
    wf_report_parser.add_argument('-f', '--time_field', default="updated_on")
    wf_report_parser.set_defaults(func=report)
    daily_report_parser = stats_subparser.add_parser('get_daily_report', help='Get a daily report for fireworks', parents=[stats_parent_parser])
    daily_report_parser.add_argument("-f", '--time_field', default="time_end")
    daily_report_parser.set_defaults(func=report)
    group_fizzled_fw_parser = stats_subparser.add_parser('group_fizzled_fws', help='Group fizzled fireworks', parents=[stats_parent_parser])
    ...
    identify_catastrophes_parser = stats_subparser.add_parser('identify_catastrophes', help='Get days with higher failure ratio', parents=[stats_parent_parser])
